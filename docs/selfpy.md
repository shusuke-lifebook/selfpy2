...
- プログラム上で扱う対象をオブジェクト(モノ)に見立てて、オブジェクトを中心にコードを組み立てていく手法を**オブジェクト指向プログラム**という

### 📒 10.1 クラスの定義

- オブジェクト指向プログラミングで中心となるのは**クラス**です。

#### 📒 10.1.1 最も簡単なクラス

- 新たにクラスを定義するのはclass命令の役割です。
  ```Python
  class Person:
      pass
  ```
...
- クラス命名する上で注意すべきポイント
  ...
  (3) 対象／機能が明確になるような単語を選定する

#### 📒 10.1.2 インスタンス変数

- **インスタンス変数**: インスタンス(オブジェクト)に属する変数のこと
  - **属性(アトリビュート)**とも言う。
  - インスタンス変数を準備するのは、**初期化メソッド**の役割です。
  - 初期化メソッドの名前は **init**(前後のアンダースコアは2個)で固定
    - 引数の先頭はselfというオブジェクトを渡す
    - selfは現在生成しようとしているインスタンスそのものを表します。
    - インスタンス変数の生成
      - self.インスタンス変数 = 値
    - **インスタンス変数へのアクセス**
      - インスタンス変数には、ドット演算子(.)を使ってアクセスできる
    ...
    - **インスタンス毎に変数を追加する**
      - いったん作成したインスタンス対して、あとからインスタンス変数を追加することができる。
  ...
  - **インスタンス変数の追加を制限する --スロット**
    - Pythonでは**スロット**という機能を提供している。スロットを利用することで、クラスが持てるインスタンス変数をあらかじめ固定(制限)できるようになる。
...
- **シングルトンパターン**
  - クラス変数の利用は原則として以下のようにごく限られた状況にとどめておくべき。
    - 読み取り専用(定数)
    - さもなくば、クラス自体の状態を監視する
- **クラス変数をインスタンス経由で操作する**
  - クラスメソッドと同じく、クラス変数もまた、インスタンス経由で参照可能

#### 📒 10.1.7 クラス変数／インスタンス変数の型アノテーション

```Python
class クラス名:
  クラス変数名: ClassVar[型名] = 限定値
  インスタンス変数名: 型名 = 既定値
```
...
- **カプセル化**の基本は、「使い手に関係ないものは見せない」です。

#### 📒 10.2.2 インスタンス変数の隠蔽

- インスタンス変数をそのまま外からアクセスできる状態にしておくことは望ましくない
  - 読み書きの許可／禁止を制御できない
  - 値の妥当性を検証できない
  - 内部状態の変更に左右される
- インスタンス変数の名前を「\_\_」(アンダースコア2個)ではじまるように命令するだけです。
- **隠蔽する際の注意点**
  - 完全に隠蔽できるわけではない
  - Pythnonは「\_\_」つきのインスタンス変数を内部的にリネームしているだけで、アクセスそのものを制御しているわけではない
  - 具体的には「\_\_」のインスタンス変数は「\_<クラス名><元の名前>」のようにリネームされる

#### 📒 10.2.3 アクセサーメソッド

- 隠蔽されたインスタンス変数にアクセスするためのメソッドのことを「**アクセサーメソッド**」という
- 一般的にアクセサーメソッドの名前は、
  - **変数の先頭から「\_\_」を取り除いて、代わりに「get\_」「set\_」を付与**

    ```Python
    def get_変数名(self):
      return self.変数名

    def set_変数名(self, value):
      self.変数名 = value
    ```

#### 📒 10.2.4 プロパティ

- Pythonは**クラスの内部ではメソッドのように定義できるが、外からは変数のようにアクセスできる**仕組みを用意している。これが**プロパティ**である。
- プロパティを設定するには、ゲッター／セッターから「get*」「set*」を取り除き代わりに、以下を付与する
  - ゲッターには@propertyデコレーター
  - セッターには@<名前>.setterデコレーター
- **property関数**
  - @property/@<名前>.setterの代わりに、property関数を用いることもできる
  - property関数
    ...
    fget: ゲッター
    fset: セッター
    fdel: del演算子によって呼びだせるメソッド
    doc: docstringを表す文字列
    ```

### 📒 10.3 継承

- **継承**とは、もとになるクラスのメンバーを引き継ぎながら、新たな機能を加えたり、元の機能を上書きする仕組みです。
...
- 継承を利用することで、基底クラスで定義したメソッドを派生クラスで上書きすることができる。これをメソッドの**オーバーライド**という

#### 📒 10.3.3 superによる基底クラスの参照

- super関数を用いることで、派生クラスから基底クラスのメソッドを呼び出すことができる。
  ```Python
  super().メソッド名(引数,...)
  ```
...
- Pythonでは継承に関してシグニチャの妥当性、制約を課すための@override、@finalデコレータを用意している
- @overrideデコレーター
  - そのメソッドが基底クラスで定義されたメソッドをオーバーライドしていることを明示的に宣言できる。
- @finalデコレーター
  - 継承、オーバーライドを禁止する

#### 📒 10.3.5 多重継承とメソッドの検索順序

- 基底クラスとして指定された順にメソッドを検索する
- 現在のクラスに対するメソッド検索順序は**mro**メソッドで確認できる。

#### 📒 10.3.6 委譲

- **委譲**では再利用したい機能を持つオブジェクトを現在のクラスのインスタンス変数として取り込む。

#### 📒 10.3.7 ミックスイン

- **ミックスイン**とは再利用可能な機能を束ねたクラスのことです。

#### 📒 10.3.8 パターンマッチング(クラスパターン)

- **継承関係にあるクラスの判定**
  - クラスパターンによる判定は、instance関数のそれに準じます。

### 📒 10.4 ポリモーフィズム

- **ポリモーフィズム**は**多態性**と訳されるが、日本語に訳しても抽象的なところがポリモーフィズムを難しく見せいている原因のようです。
- ポリモーフィズムとは要は、「同じ名前のメソッドで異なる挙動を実現すること」をいう。

#### 📒 10.4.1 ポリモーフィズムの基本

- ポリモーフィズムのメリットは、同じ目的の機能を呼び出すために異なる名前(命令)を覚えなくてもよい点です。
  - Triangleクラスで面積を求めるのはget_areaメソッド
  - Rectangleクラスで面積を求めるのはcalculate_areaメソッド
...
- 例外処理とはあらかじめ発生するかもしれないエラーを想定しておき、そのエラーが発生した場合に行うべき処理のことを言う。

#### 📒 11.1.1 例外クラスの型

- 一般的な例外クラスは、Exceptionクラスを基底クラスとする階層ツリーの中に属している。
- **except節の記述順序**
  - 複数のexcept節がある場合には、記述が先にあるものが優先される。

#### 📒 11.1.2 終了処理を定義する - finally節

- finally節は、例外の有無にかかわらず最終的に実行される節です。
- **with ... as命令**
  - with ... as命令はリソース破棄に特化した仕組みなので、try ... finally命令がシンプルかつ明確に表現できる
  - **コンテキストマネージャー**
    - with ... as命令にオブジェクトが渡せるかどうかは、オブジェクトが**コンテキストマネージャー**に対応しているかできまる。

      | メソッド                        | 概要                                                                                                                                                                                                |
      | :------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
      | **enter**(self)                 | with節に入ったときに呼び出される。リソース(コンテキスト)を準備。戻り値はasで指定された変数に反映される。                                                                                            |
      | **exit**(self, type, value, tb) | with節を抜けるときに呼び出され、リソース(コンテキスト)を破棄。type/value/tbには、with節内で例外が発生した場合に、その型、値、トレースバックが渡される。例外が発生した場合、戻り値がTrueで例外を無視 |

- **finally節の実行順序**
  - try → (あれば) except → finallyの順で節が実行される。

#### 📒 11.1.3 例外をスローする - raise命令

- raise命令を利用することで、アプリ開発者が自ら例外を発生させることが可能です。
  ```Python
  raise 例外オブジェクト
  ```
- **例外発生させる場合の注意点**
...
- 例外クラスは、アプリ独自に定義することができる
- 例外クラスの作り方
  - (1) 標準のExceptionクラス(または、その発生クラス)を継承すること
  - (2) クラス名の接尾辞は、Errorとすること
  - (3) アプリ(モジュール)が複数の例外を持つ場合は、アプリ独自例外の基底クラスを準備しておき、他の例外はこれを継承すること
  - (4) Exception(またはその派生クラス)を継承するだけで中身を空にする
  - (5) さもなくば、except節で例外処理する際のキーとなる最低限の情報だけ持たせる。
...
- Pythonではあらかじめ特定の役割を与えられたメソッドとして、**特殊メソッド**が用意されている。
- 前後アンダースコア2個でくくっているのが目印(\_\_)

#### 📒 11.2.1 オブジェクトの文字列表現を取得する

- **\_\_str\_\_**メソッドは可能であれば、すべてのクラスに実装すべきです。オブジェクトの適切な文字列表現を用意しておくことで、ロギング／単体テストなどの局面でも、print(obj)とするだけでオブジェクトの概要が確認できる。
- **解析可能な表現を取得する**
  - \_\_repr\_\_メソッド： eval関数が呼び出されたときに元のオブジェクトを復元できるような文字列を返すことが期待される。repr関数にオブジェクトが渡された場合などに呼び出される。

#### 📒 11.2.2 オブジェクト同士が等しいかどうかを判断する

- **\_\_eq\_\_**メソッドは、オブジェクトの同値性を判定するメソッドです。
- 「==」判定されたときに呼び出される。
- 「==」演算子のオペランドが継承関係にある場合
  - **左辺右辺いずれに位置するにもかかわらず、派生クラスの\_\_eq\_\_メソッドによって同値判定**する。

#### 📒 11.2.3 オブジェクトのハッシュ値を取得

- \_\_hash\_\_メソッドは、オブジェクトのハッシュ値、オブジェクトのデータを元に生成されたint値を返します。
- オブジェクトの同値判定に関わる情報をタプルにまとめ、hash関数に渡すだけです。これでオブジェクト全体としてハッシュ値を求められる
- **hashable であることの条件**
...
- データだけ扱うクラス。Python3.7以降では、そのようなクラスを表現するための仕組みを用意している。それが**データクラス**である。
- データクラスには以下のメリットがある。
  - (1) 基本メソッドを自動生成してくれる。
  - (2) イミュターブルなオブジェクトを生成できる
  - (3) メソッドは自由に追加できる

#### 📒 11.3.1 データクラスの基本

- @dataclass デコレータを付与する
- データクラスを宣言するには、従来のクラス宣言に@dataclasses.dataclassデコレーターを付与するだけ。
- フィールドを宣言する
  - データクラスが保持するインスタンス変数(**フィールド**とも言う)
    ```Python
    フィールド名: 型(名前)
    ```
